...
Первичная сверка production с GitHub уже выполнена. Сервер находится на обычном
чистом checkout ветки `rebuild-erp-core`, поэтому последующие проверенные релизы
можно устанавливать командой `bash deploy.sh`. Скрипт намеренно не делает
`reset --hard`, не прячет изменения через stash и не выполняет слепой `git pull`.
