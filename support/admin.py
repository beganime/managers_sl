...
class SupportMessageAdmin(ModelAdmin):
    ...
    def subject_card(self, obj):
        return format_html(
            ...
            '<div style="font-weight:800;font-size:14px;margin-bottom:4px">{}</div>'
            '<div style="font-size:12px;color:#64748b;max-width:420px;white-space:normal">{}</div>'
            '</div>',
            obj.subject,
    ...
    def user_display(self, obj):
        ...
        return format_html(
            '<div><b>{}</b><br><span style="color:#64748b;font-size:12px">{}</span></div>',
            label,
            obj.user.email,
        )
    ...
    def attachment_preview(self, obj):
        ...
        if obj.photo:
            photo_url = escape(obj.photo.url)
            parts.append(f'<a href="{photo_url}" target="_blank" rel="noopener noreferrer">📷 Фото</a>')

        if obj.file:
            file_url = escape(obj.file.url)
            parts.append(f'<a href="{file_url}" target="_blank" rel="noopener noreferrer">📎 Файл</a>')
    ...
    def photo_preview_large(self, obj):
        ...
        return format_html(
            ...
            '<img src="{}" style="max-width:420px;max-height:320px;border-radius:16px;border:1px solid #e5e7eb;object-fit:cover" />'
            '</a>',
            obj.photo.url,
            obj.photo.url,
        )
    ...
    def file_link(self, obj):
        ...
        return format_html(
            '<a href="{}" target="_blank" rel="noopener noreferrer" style="font-weight:700">📎 Скачать / открыть файл</a>',
            obj.file.url,
        )
