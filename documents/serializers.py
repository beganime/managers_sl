...
class DocumentTemplateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'is_active', 'updated_at', 'fields_config',
        )
...
class GeneratedDocumentSerializer(serializers.ModelSerializer):
    ...
    can_download  = serializers.BooleanField(read_only=True)  # ← из @property модели

    class Meta:
        model  = GeneratedDocument
        fields = '__all__'
        read_only_fields = (
            'manager', 'status', 'generated_file',
            'approved_by', 'approved_at',
            'created_at', 'updated_at',
        )

    def get_file_url(self, obj):
        # Возвращаем URL только если документ одобрен
...
class KnowledgeTestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = KnowledgeTest
        fields = ('id', 'title', 'description', 'questions', 'updated_at')