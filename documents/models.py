...
# Настройка локали для русских дат (по желанию, можно и просто форматировать)
try:
    locale.setlocale(locale.LC_ALL, 'ru_RU.UTF-8')
except:
    pass # Если на Windows нет такой локали

from clients.models import Client
from catalog.models import Program

# --- 1. БАЗА ЗНАНИЙ (Оставляем как было) ---
...
class ContractTemplate(models.Model):
    TYPE_CHOICES = (
        ('education', 'Обучение (Студент)'),
        ('work', 'Рабочая виза'),
        ('consent', 'Согласие на обработку'),
    )
    title = models.CharField("Название шаблона", max_length=255)
    type = models.CharField("Тип документа", max_length=20, choices=TYPE_CHOICES, default='education')
...
class Contract(models.Model):
    ...
    # Данные Заказчика (Если платит не студент, а родитель)
    # Если пусто - берем данные Студента
    ...
    def generate_document(self):
        ...
        price = self.custom_price if self.custom_price else (self.program.price if self.program else 0)
        
        # 2. Определяем Заказчика (Родитель или сам Студент)
        ...
        # 3. Формируем контекст (ВСЕ ТЕГИ СЮДА)
        context = {
            # --- ОБЩИЕ ДАННЫЕ ---
            'id': self.id,
            ...
            'passport_num': self.client.passport_inter_num, # Обычно в договорах загран
            ...
            # --- ДАННЫЕ ЗАКАЗЧИКА (Того кто платит) ---
            ...
            # --- ФИНАНСЫ ---
            'amount': f"{price:,.2f}", # Цена с разделителями
            'payment_deadline': self.payment_deadline.strftime("%d.%m.%Y") if self.payment_deadline else "__________",
            'program_name': self.program.title if self.program else "Услуги компании",
        }
