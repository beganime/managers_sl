...
def close_overdue_shifts():
    """
    Безопасное авто-закрытие забытых смен.
    Идемпотентно: повторный вызов не портит данные.
    """
    ...
    for shift in overdue_shifts:
        should_close = (
            shift.date < today or
            (shift.date == today and now.time() >= datetime.time(22, 0))
        )
        ...
        close_dt = timezone.make_aware(
            datetime.datetime.combine(shift.date, datetime.time(22, 0)),
            tz
        )
        ...
        auto_closed_count = WorkShift.objects.filter(
            employee=employee,
            is_auto_closed=True
        ).count()
...
def dashboard_callback(request, context):
    ...
    if is_admin_user(user):
        ...
        context.update({
            'kpi': [
                ...
                },
                {
                    ...
                    'footer': 'Подтверждённые платежи - расходы',
                    'color': 'success',
                },
                {
                    'title': 'Активные сделки',
                    'metric': active_deals,
                    'footer': 'Новые / ждут оплату / частично оплачены',
                    'color': 'warning',
                },
                {
                    'title': 'Клиентов всего',
                    'metric': total_clients,
                    'footer': 'Вся клиентская база',
                    'color': 'info',
                },
                {
                    'title': 'Ждут подтверждения',
                    'metric': pending_pays,
                    'footer': f'Платежи: {pending_pays} | Документы: {pending_docs}',
                    'color': 'danger',
                },
            ],
            'chart': {
        ...
        context['has_active_shift'] = WorkShift.objects.filter(
            employee=user,
            date=today,
            is_active=True
        ).exists()
        context['has_report_today'] = DailyReport.objects.filter(
            employee=user,
            date=today
        ).exists()
        context['forgets_count'] = WorkShift.objects.filter(
            employee=user,
            is_auto_closed=True
        ).count()

        context['new_leads'] = Lead.objects.filter(
        ...
        context['my_deals'] = Deal.objects.filter(
            manager=user
        ).order_by('-updated_at')[:5]

        context['my_tasks'] = Task.objects.filter(
            assigned_to=user
        ).exclude(status='done').order_by('deadline', '-updated_at')[:5]
        ...
        context.update({
            'kpi': [
                {
                    'title': 'ЗП (Оклад + Бонус)',
                    'metric': f'${balance + fixed:,.2f}',
                    'footer': f'Оклад: ${fixed:,.0f} | Бонус: ${balance:,.0f}',
                    'color': 'success',
                },
                {
