...
class DocumentTemplateAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Jinja-переменные",
            {
                ...
                "description": "Поля шаблона добавляйте ниже через inline-блоки после первого сохранения шаблона. JSON вручную заполнять не нужно.",
            },
        ),
        (
        ...
        (
            "Электронная печать и водяной знак",
            {
                ...
                "description": "Печать и водяной знак лучше добавлять после первого сохранения шаблона. Если печать не настроена, генерация PDF покажет понятную ошибку.",
            },
        ),
        (
    ...
    def stamp_help(self, obj):
        return format_html(
            ...
            "<b>Печать:</b> после первого сохранения шаблона можно добавить Stamp rule: загрузить изображение, указать ширину/высоту в мм и позицию bottom_left, bottom_right, top_left, top_right, center или custom.<br>"
            "<b>Custom:</b> заполните x_mm и y_mm.<br>"
            "<b>Водяной знак:</b> включите watermark_enabled, укажите текст или изображение, позицию и прозрачность.<br>"
            "<b>Важно:</b> если печать не нужна, не добавляйте Stamp rule."
            "</div>"
        )
