...
class OnboardingApiTests(APITestCase):
    ...
    def test_manager_approval_creates_client_questionnaire_and_canonical_applications(self):
        created = self.create_submission()
        submission = OnboardingSubmission.objects.get(public_id=created.data['public_id'])
        self.client.force_authenticate(self.manager)
        ...
        applications = Application.objects.filter(client=submission.client).order_by('university_id')
        self.assertEqual(applications.count(), 3)
        for application, university, program in zip(applications, self.universities, self.programs):
            self.assertIsNotNone(application.public_id)
            self.assertEqual(application.university, university)
            self.assertEqual(application.program, program)
            self.assertEqual(application.country_reference, university.country)
            self.assertEqual(application.academic_year, 2027)
            self.assertEqual(application.program_name, program.name)
    ...
    def test_each_selected_program_gets_its_own_application(self):
        second_program = Program.objects.create(
            university=self.universities[0],
            name='Дополнительная программа',
        )
        payload = self.applicant_payload()
        payload['university_choices'][0]['program_ids'].append(second_program.id)
        created = self.create_submission(payload)
        submission = OnboardingSubmission.objects.get(public_id=created.data['public_id'])
        self.client.force_authenticate(self.manager)
        response = self.client.post(
            reverse('manager-onboarding-submission-review', kwargs={'pk': submission.pk}),
        ...
        self.assertEqual(response.status_code, 200, response.data)
        submission.refresh_from_db()
        applications = Application.objects.filter(client=submission.client)
        self.assertEqual(applications.count(), 4)
        self.assertEqual(applications.filter(university=self.universities[0]).count(), 2)
        self.assertEqual(applications.values('university_id', 'program_id').distinct().count(), 4)
