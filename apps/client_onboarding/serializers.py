...
class OnboardingSubmissionWriteSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        from .telegram import enqueue_new_application
        transaction.on_commit(lambda: enqueue_new_application(submission.pk))
        return submission
...
class PublicOnboardingStatusSerializer(serializers.ModelSerializer):
    ...
    def get_service_credentials(self, obj):
        ...
        credentials = {
            ...
            'activation_mode': 'onboarding_token' if not identity.shared_password else 'legacy_password',
        }
        if identity.shared_password:
            credentials['shared_password'] = identity.shared_password
        return credentials
...
class ReviewDecisionSerializer(serializers.Serializer):
    decision = serializers.ChoiceField(choices=('start_review', 'process', 'approve', 'request_changes', 'reject'))
